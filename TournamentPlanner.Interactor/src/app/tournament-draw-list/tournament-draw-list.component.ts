...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatCardModule, MatButtonModule, MatChipsModule, MatCommonModule],
...
export class TournamentDrawListComponent implements OnInit {
  ...
  async ngOnInit() {
    try {
      var reqResponse = await this._tpService.getTournamentDraws(this.tournamentId!);
      ...
      console.log(error);
      console.log((error as any).error ?? (error as any).error?.Error ?? "An unknown error occurred.");
    }
  }
