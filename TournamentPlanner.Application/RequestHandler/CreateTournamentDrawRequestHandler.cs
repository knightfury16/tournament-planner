...
public class CreateTournamentDrawRequestHandler
    : IRequestHandler<CreateTournamentDrawRequest, IEnumerable<DrawDto>>
{
    private readonly IRepository<Tournament> _tournamentRepository;
    private readonly IMapper _mapper;
    private readonly ITournamentService _tournamentService;

    public CreateTournamentDrawRequestHandler(
    ...
    public async Task<IEnumerable<DrawDto>?> Handle(
        CreateTournamentDrawRequest request,
        CancellationToken cancellationToken = default
    )
    {
        //!! Axiom:: in TP I can only make two draw at most if the type is Group
        //does the tournament exists?
        var navigationProperty = Utility.NavigationPrpertyCreator(
            nameof(Tournament.Draws),
            nameof(Draw.MatchType)
        );
        var tournament = (
            await _tournamentRepository.GetAllAsync(
                t => t.Id == request.TournamentId,
                [nameof(Tournament.Participants), navigationProperty]
            )
        ).FirstOrDefault();
        ...
        if (tournament.Status <= TournamentStatus.RegistrationOpen)
        {
            throw new BadRequestException(
                "Please close the Tournament Registration in order to make draw"
            );
        }
        //can i make draw?
        ...
        if (!canIDarw)
            throw new BadRequestException("Previous draws are not completed.Can not make new draw");

        //make draw -> make matchType
        var draws = await _tournamentService.MakeDraws(
            tournament,
            request.MatchTypePrefix,
            request.SeedersId
        );
