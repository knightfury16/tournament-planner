...
public class AddMatchScoreRequestHandler : IRequestHandler<AddMatchScoreRequest, MatchDto>
{
    ...
    )
    {
        ...
        var tournamentNavigationProp = Utility.NavigationPrpertyCreator(
        ...
        var match = await _matchRepository.GetByIdAsync(
            request.MatchId,
            [
                tournamentNavigationProp,
                nameof(Match.FirstPlayer),
                nameof(Match.SecondPlayer),
                nameof(Match.Round),
            ]
        );
        ...
        if (!_tournamentService.AmITheCreator(match.Tournament))
        ...
        IScore gameScore = gameTypeHandler.DeserializeScore(
            request.AddMatchScoreDto.GameSpecificScore
        );
