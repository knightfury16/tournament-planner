...
public class MakeTournamentMatchScheduleRequestHandler
    : IRequestHandler<MakeTournamentMatchScheduleRequest, IEnumerable<MatchDto>>
{
    ...
    public MakeTournamentMatchScheduleRequestHandler(
        IRepository<Tournament> tournamentRepository,
        IMapper mapper,
        IDrawService drawService,
        IMatchService matchService,
        ITournamentService tournamentService,
        IMatchScheduler matchScheduler
    )
    {
    ...
    public async Task<IEnumerable<MatchDto>?> Handle(
        MakeTournamentMatchScheduleRequest request,
    ...
    )
    {
        ...
        var nagivationProperty = Utility.NavigationPrpertyCreator(
            nameof(Tournament.Draws),
            nameof(Draw.MatchType),
            nameof(MatchType.Players)
        );
        var tournament = await _tournamentRepository.GetByIdAsync(
            request.TournamentId,
            [nagivationProperty, nameof(Tournament.Matches)]
        );
        if (tournament == null)
        ...
        if (canISchedule == false)
            throw new BadRequestException(
                "Draws not made or previous draws not complete. Can not make schedule"
            );

        //go to match service with all the draws
        var matches = await _matchService.CreateMatches(tournament, request.SchedulingInfo);

        var scheduledMatches = await _matchScheduler.DefaultMatchScheduler(
            matches.ToList(),
            request.SchedulingInfo
        );
