        ...
        public static List<Match> MakeAllMatchRadomWinner(List<Match> matches)
        {
            ...
            for (var i = 0; i < matches.Count; i++)
            {
                if (matches[i].IsComplete == false)
                {
                    ...
                    matches[i].GamePlayed = matches[i].GameScheduled?.ToUniversalTime();
                }
            }
            return matches;
        }
        ...
        public static List<Match> MakeSomeMatchRandomWinner(List<Match> matches, int matchIndex = 0)
        {
            ...
            for (var i = 0; i < randomMatchWinnerNumber; i++)
            {
                ...
                if (matches[randomMatchIndex].IsComplete == false)
                {
                    ...
                    //* game played on scheduled
                    matches[randomMatchIndex].GamePlayed = matches[randomMatchIndex].GameScheduled?.ToUniversalTime();
                }
            }
            return matches;
        }
